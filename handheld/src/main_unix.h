...
// Simple Unix (X11 + EGL + GLES) entry point that mirrors the Win32/RPi
// structure but targets generic Linux desktops.
...
// X11 uses typedefs named Screen and Font, which conflict with Pewiku's
// own GUI classes. Rename them locally for this TU.
...
static bool loadNetWmIcon(const std::string& path, std::vector<unsigned long>& iconData) {
	...
	png_set_read_fn(pngPtr, (png_voidp)&source, [](png_structp ptr, png_bytep data, png_size_t len) {
		((std::istream*)png_get_io_ptr(ptr))->read((char*)data, len);
	});
	...
	iconData.push_back((unsigned long)width);
	iconData.push_back((unsigned long)height);
...
int main(int argc, char** argv) {
	// Change working directory to the binary location, so relative paths like
	// "data/images/..." work in the same way as on other platforms.
	if (argc > 0 && argv[0]) {
	...
	if (!dpy) {
		std::fprintf(stderr, "Failed to open X11 display\n");
		return 1;
	}
	...
	if (loadNetWmIcon("data/app/ios/icons/Icon.png", netWmIcon)) {
		...
		if (netWmIconAtom != None) {
			XChangeProperty(
				...
				32,
				PropModeReplace,
				(const unsigned char*)netWmIcon.data(),
				(int)netWmIcon.size()
			);
		}
	}
	...
	// EGL init (OpenGL ES 1-style context, like other platforms)
	...
	if (!eglBindAPI(EGL_OPENGL_ES_API)) {
		std::fprintf(stderr, "eglBindAPI failed\n");
		return 1;
	}
	...
	appContext.platform = nullptr;
	appContext.doRender = true;

	// Simple platform implementation reusing the Linux texture loader from RPi path
	...
	public:
		...
		TextureData loadTexture(const std::string& filename_, bool textureFolder) {
			...
			std::string filename = textureFolder ?
				std::string("data/images/") + filename_ :
				filename_;
